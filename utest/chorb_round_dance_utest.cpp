...
SCENARIO("Swapping dancers works correctly") {
  GIVEN("A round dance with 5 dancers") {
    ChorbRoundDance* dance = createTestChorbRoundDance(5);
    ...
    WHEN("The 2nd dancer releases the 3rd dancer") {
      ...
      AND_WHEN("The 5th dancer releases the 4th dancer") {
        dance->release("dancer5", Direction::LEFT);

        THEN(
            "The swap of 3rd and 4th dancers should be successful, regardless "
            "of "
  ...
  GIVEN("A round dance with 3 dancers") {
    ...
    WHEN("We swap the 2nd and 3rd dancers") {
      dance->release("dancer1", Direction::BOTH);
      REQUIRE(dance->swap("dancer3", "dancer2"));

      THEN(
          "The dancers should have correct nicknames and be in correct "
          "order") {
        // We should have d1 <--> d3 <--> d2
        auto dancers = dance->getDancers();
        auto it = dancers.begin();
        REQUIRE(it->getNickname() == "dancer1");
        REQUIRE(it->getLeftDancer()->getNickname() == "dancer2");
